...
const handleOpen = () => setOpen(!open);
  ...
  return (
   ...
   {examdata.map((el:any)=>(
  <MenuItem value={el.exam_name}>{el.exam_name} - {el.session}</MenuItem>
   ))}
    ...
    <label className="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
        <div className="flex flex-col items-center justify-center pt-5 pb-6">
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400">Excel file only</p>
        </div>
        <input id="dropzone-file" type="file"         accept=".xlsx, .xls"  name='file' onChange={handleFileChange1} className="hidden" />
    </label>

</div> 
<p className="text-xs text-red-500 dark:text-gray-400 text-center">* Please Upload Only Excel File</p>
<p className="text-xs text-red-500 dark:text-gray-400 text-center">* Please Follow Correct Excel Sequences</p>


<br />
<Button color='green' onClick={HandlePostData} className='w-full m-auto text-center '>{btnload?<Spinner className='w-6 h-6 m-auto'/>:"Upload"}</Button>
        ...
        />
            ...
            <p className='font-bold text-center text-2xl'>Students Marks Successfully Upload</p>
